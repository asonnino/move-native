...
impl<'a, 'b, 'ctx> ControlFlowEmitter<'a, 'b, 'ctx> {
    ...
    /// Load the tag integer from an enum value or reference.
    fn load_enum_tag(&self, source: usize) -> CompileResult<inkwell::values::IntValue<'ctx>> {
        let llvm = self.state.ctx();
        match &self.state.get_local(source)?.mty {
            Type::Reference(_, inner) => match inner.as_ref() {
                Type::Datatype(..) => {
                    ...
                    let tag_type = enum_type.get_field_type_at_index(0).ok_or_else(|| {
    ...
    pub(super) fn emit(&self, byte_code: &Bytecode) -> CompileResult<()> {
        ...
        match byte_code {
            ...
            Bytecode::VariantSwitch(_, source, labels) => {
                let tag = self.load_enum_tag(*source)?;
                let default_block =
