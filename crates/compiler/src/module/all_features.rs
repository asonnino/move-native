...
impl CompiledModuleBuilder {
    ...
    /// | 2     | `forty_two`       | constant loading                             |
    /// | 3     | `low_byte`        | integer cast (u64 → u8)                      |
    /// | 4     | `cast_widths`     | integer width casts (U16/U32/U128/U256)      |
    /// | 5     | `add_u16`         | u16 in signature + arithmetic                |
    /// | 6     | `add_u32`         | u32 in signature + arithmetic                |
    /// | 7     | `add_u128`        | u128 in signature + arithmetic               |
    /// | 8     | `add_u256`        | u256 in signature + arithmetic               |
    /// | 9     | `discard`         | Pop (Destroy) to discard a value             |
    /// | 10    | `swap_fields`     | mutable field borrows, ReadRef, WriteRef     |
    /// | 11    | `freeze_and_read` | MutBorrowField → FreezeRef → ReadRef         |
    /// | 12    | `read_x`          | ImmBorrowField → ReadRef                     |
    /// | 13    | `swap_u64`        | multi-return                                 |
    /// | 14    | `checked_sub`     | comparison + conditional abort               |
    /// | 15    | `round_trip`      | intra-module calls (make_point → sum_point)  |
    /// | 16    | `identity<T>`     | generic function                             |
    /// | 17    | `call_identity`   | CallGeneric (identity\<u64\>)                |
    /// | 18    | `sum_loop`        | while loop + calls + struct interaction      |
    /// | 19    | `integration_test`| chains sum_loop → call_identity → swap_u64 → checked_sub |
    ...
    pub fn all_features() -> (CompiledModule, Vec<CompiledModule>) {
        ...
        let builder = Self::new()
            ...
            .function(
                ...
                vec![
                    CopyLoc(0),                    // 0: push n
                    Call(FunctionHandleIndex(18)), // 1: sum_loop(n)
                    StLoc(1),                      // 2: total
                    CopyLoc(1),                    // 3
                    Call(FunctionHandleIndex(17)), // 4: call_identity(total)
                    StLoc(2),                      // 5: id
                    CopyLoc(2),                    // 6
                    LdU64(0),                      // 7
                    Call(FunctionHandleIndex(13)), // 8: swap_u64(id, 0) → (0, id)
                    StLoc(4),                      // 9: b = id (second return)
                    StLoc(3),                      // 10: a = 0 (first return)
                    CopyLoc(4),                    // 11: push b
                    CopyLoc(3),                    // 12: push a
                    Call(FunctionHandleIndex(14)), // 13: checked_sub(b, a)
                    Ret,                           // 14
                ],
            );
